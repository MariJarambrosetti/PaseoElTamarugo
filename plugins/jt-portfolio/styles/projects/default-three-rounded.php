...
<div class="uk-grid-width-small-1-2 uk-grid-width-medium-1-3 uk-grid-width-large-1-3 default-3-rounded uk-grid-match" data-uk-grid="{controls: '#filter-list'}">
	<?php
	...
	<div <?php if ($display_filters == 'yes') { ?>data-uk-filter="<?php echo $project_filter; ?>" <?php } ?>>
		<figure class="uk-overlay uk-overlay-hover">			<?php $project_img = wp_get_attachment_url( get_post_thumbnail_id($post->ID), 'thumbnail'  ); ?>
			...
			?>
			<figcaption class="uk-overlay-background uk-overlay-bottom uk-overlay-slide-bottom uk-overlay-panel"><span class="project-info"><?php echo esc_attr($project_short_info); ?></span></figcaption>
			<?php
				}
			?>
		</figure>
		<p class="project-title">
			<span class="outer">
				<span class="inner"><?php echo $post->post_title; ?></span>
			</span>
		<?php
		...
		<a class="lbutton" href="<?php echo get_permalink($post->ID); ?>">
			<?php
				if ( ( !empty ($project_lbutton_text) ) || ( !empty ($project_lbutton_icon) ) )  {
			?>
		...
		?>
			<a class="rbutton" href="<?php echo esc_attr($project_rbutton_url); ?>">
				<?php
					if ( ( !empty ($project_rbutton_text) ) || ( !empty ($project_rbutton_icon) ) )  {
				?>
