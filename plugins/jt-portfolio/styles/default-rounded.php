...
<div class="uk-grid-width-small-1-2 uk-grid-width-medium-1-3 uk-grid-width-large-1-3 default-rounded uk-grid-match" data-uk-grid="{controls: '#filter-list'}">
	<?php
	...
	<div <?php if ($display_filters == 'yes') { ?>data-uk-filter="<?php echo rtrim(${'filter' . $i}); ?>" <?php } ?>>
		...
		?>
		<p class="project-title"><?php echo rtrim(${'title' . $i }); ?><span class="project-info"><?php echo rtrim(${'short_info' . $i }); ?></span></p>
		<?php
			}
		?>
	</div>
