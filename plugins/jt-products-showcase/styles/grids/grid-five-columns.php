...
?>
	<div class="uk-grid product-grid-5-cols">
		<?php
			...
			foreach($products_array as $product) {
				...
				echo '<div class="uk-width-1-5">
						...
						if ($add_to_cart_btn == 'yes') {
							echo '<p class="cart-btn"><a class="add-to-cart-btn" href="' . $cart_url . '?add-to-cart=' . $product->ID . '"><i class="fa fa-shopping-cart"></i> Add to Cart</a></p>';
						}
						if ($show_read_more == 'yes') {
							echo '<p class="product-btn"><a href="'  . get_permalink( $product->ID ) . '">' . __('Read More'). '</a></p>';
						}
					echo '</div>';
			} 
